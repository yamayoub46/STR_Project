...
# A task instance
from flaskProject2.algorithms.DM.prime import lcm
...
if __name__ == '__main__':
        html_color = {'Task1': 'red', 'Task2': 'blue', 'Task3': 'green', 'Task4': 'aqua', 'Task5': 'coral',
                      'De.ferrable': 'yellow', 'Empty': 'grey', 'Finish': 'black'}
        ...
        # Html output start
        html = "</main><div style='margin: 50px auto auto;'>"
        ...
        for i in range(0, hyperperiod, clock_step):
            ...
            if len(possible) > 0:
                ...
                html += '<div style="float: left; text-align: center; width: 110px; height: 20px; background-color:' + \
                        html_color[on_cpu.name] + ';">' + on_cpu.get_unique_name() + '<br />' + str(i) + '-' + str(
                    i + 1) + '</div>'
                if on_cpu.use(clock_step):
                    tasks.remove(on_cpu)
                    html += '<div style="float: left; text-align: center; width: 10px; height: 20px; background-color:' + \
                            html_color['Finish'] + ';">' + '<br />' + str(i + 1) + '</div>'
                    print("Finish!", )
            else:
                print('No task uses the processor. ')
                html += '<div style="float: left; text-align: center; width: 110px; height: 20px; background-color:' + \
                        html_color['Empty'] + ';">Empty' + '<br />' + str(i) + '-' + str(i + 1) + '</div>'
            print("\n")
        ...
        for line in lines:
            indexHtml += line
        output = open('../../outputs/dm.html', 'w')
        output.write(indexHtml + html)
        output.close()
